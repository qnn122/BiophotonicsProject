...
prompt = {'Enter port name: ', 'Sampling Frequency: ', 'Communication type (Serial or Bluetooth): '};
def = {'Chau_HC-05', '200', 'Bluetooth'};
...
%     set(h_axes1,'NextPlot','add'); %
    grid(h_axes1, 'on');
%     set(h_axes1,'NextPlot','new'); %

    plot(h_axes1, xTime,yDataCH1,'b-','LineWidth',2) ;
...
function analyzeButton_Callback(hObj, event, handles)
    ...
    [y, HR, peakLoc] = calcHR(sig, Fs);
    peakLoc(find(peakLoc>1000)) = [];   % reject all peak locations that are out of bound
    assignin('base', 'peakLoc', peakLoc);
    
    % Display result
    cla(h_axes2)
    plot(h_axes2, time/Fs, sig); 
    hold on;
    plot(h_axes2, time/Fs, y+mean(sig), '--', 'Color', [1 0.5 0.5], 'LineWidth',3);
    plot(h_axes2, (peakLoc+time(1))/Fs, sig(peakLoc), 'gx', 'MarkerSize', 15, 'LineWidth',2);
    axis tight
